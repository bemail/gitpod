...
func parseProcessTree() (repoUrl string, gitCommand string) {
	...
	repoUrlRegExp := regexp.MustCompile(`origin\s*(https:.*\.git)\n`)
	pid := os.Getpid()
	for {
		...
		if gitCommand != "" {
			match := gitCommandRegExp.FindStringSubmatch(strings.ReplaceAll(cmdLine, string(byte(0)), " "))
			if len(match) == 3 {
				gitCommand = match[2]
			}
		}
		if repoUrl != "" {
			match := repoUrlRegExp.FindStringSubmatch(cmdLine)
			if len(match) == 2 {
				repoUrl = match[1]
			}
		}
